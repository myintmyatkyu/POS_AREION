...
{
    public class PaymentDataUtility
    {
        public int Insert(CustomerPayment payment)
        {
            try
            {
                ...
                sqlcmd.CommandText = "Insert into PaymentCustomer(ReceiptNo,CustomerID,Date,CreditAmount,PaidAmount,Balance,Remarks,ReceivedBy,PaidBy) "+
                                                             "values (@ReceiptNo,@CustomerID,@Date,@CreditAmount,@PaidAmount,@Balance,@Remarks,@ReceivedBy,@PaidBy)";
                sqlcmd.Parameters.AddWithValue("@ReceiptNo", payment.ReceiptNo);
                sqlcmd.Parameters.AddWithValue("@CustomerID", payment.Customer.CustomerID);
