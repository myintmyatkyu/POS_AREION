...
{
    public partial class frmPurchaseByItem : Form
    {
        public frmPurchaseByItem()
        {
            InitializeComponent();
        ...
        {
            dgv.Rows.Clear();
            foreach (PurchaseDetail detail in PurchaseDetailService.GetAllPurchases(dtpFrom.Value.Date, dtpTo.Value.Date,cboItemCode.SelectedValue.ToString()))
            {
                PurchaseHeader sh = PurchaseHeaderService.GetPurchaseHeader(detail.VoucherNo);
                dgv.Rows.Add(new object[] { sh.VoucherNo, sh.Date.ToString("dd-MM-yyyy"), sh.Supplier.Name,  detail.Qty, detail.UOM.Code, detail.Price, detail.NetAmount });
            }
